...
@section('content')
  ...
  <div class="w-full flex justify-center  px-10">
    ...
    <form action="{{route('register')}}" method="POST" class="space-y-3 py-3 w-full">
        <div class="flex w-full justify-center text-xl md:text-2xl font-bold text-gray-300">Sign Up</div>

        @include('components.create-user-form')

        <div class="flex w-full pt-5">
            <button type="submit" class="w-full bg-blue-900 hover:bg-blue-800 px-4 py-1 rounded text-white font-bold text-lg">Sign Up</button>
        </div>

        <div class="flex w-full justify-center text-gray-300 text-xl">Or</div>
